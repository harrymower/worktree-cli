...
import { assignSlot, releaseSlot, getSlot, loadSlots, calculateHostPort } from './ports.js';
...
function getComposeFilename(worktreeName: string | null): string {
  if (!worktreeName) {
...
function getComposePath(projectRoot: string, config: MonorepoConfig, worktreeName: string | null): string {
  if (!worktreeName) {
...
function getComposeProject(config: MonorepoConfig, worktreeName: string | null): string {
...
export function createWorktree(config: WorktreeConfig, options: CreateWorktreeOptions = {}): boolean {
  ...
  try {
    slot = assignSlot(
      projectRoot,
      monorepoConfig.project.worktree_dir,
      name,
      monorepoConfig.ports.max_worktrees,
    );
  } catch (error) {
    logError(`${error}`);
    return false;
...
function printSuccessMessage(config: WorktreeConfig, slot: number): void {
...
export function cleanupAll(force: boolean = false, removeVolumes: boolean = false): void {
  const projectRoot = findProjectRoot();
  const config = loadMonorepoConfig(projectRoot);
  const worktreesDir = join(projectRoot, config.project.worktree_dir);
 ...
 */
function isYalcAvailable(): boolean {
  try {
...
function setupLocalDev(
  context: DevContext,
  options: { localCli?: boolean; localSdk?: boolean },
): void {
 ...
 */
function cleanupLocalDev(context: DevContext): void {
  console.log();
